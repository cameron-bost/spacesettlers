...
public class AtkiGAChromosome {
	...
	public AtkiGAChromosome()
	{
		...
		graph = new spacesettlers.bost7517.AStarGraph(1000, 1000, false);
	 ...
	 * @return
	 */
	public AbstractAction getCurrentAction(Toroidal2DPhysics space, Ship myShip, AtkiGAState currentState, int policyNumber) 
	{
		...
		if (!policy.containsKey(currentState)) {
				...
				if(current != null)
				{
					if(currentState.getEnergySource() != null) {
						...
						current = null; //resets current stepp to null so it is able to update step
							...
							currentPath = graph.getPathTo(myShip,  currentState.getEnergySource(), space); //Wil get the current path that a* has chosen
						...
						if(current != null && (myShip.getEnergy() > LOW_ENERGY_THRESHOLD)) // Want to make sure not to interrupt an a* move that has not finished yet.
						{
						...
						policy.put(currentState, new BDSMMoveToObjectAction(space, myShip.getPosition(), currentState.getEnergySource()));			
					}
				}
			}
			...
			if(policyNumber == 2)
			{
				...
				policy.put(currentState, new BDSMMoveToObjectAction(space, myShip.getPosition(), currentState.getBase()));	
				aimingForBase.put(myShip.getId(), true);
			...
			if(policyNumber == 3)
			{
				if (currentState.nearestMineableAsteroid != null) {
					...
					policy.put(currentState, new BDSMMoveToObjectAction(space, myShip.getPosition(), currentState.nearestMineableAsteroid,currentState.nearestMineableAsteroid.getPosition().getTranslationalVelocity()));
					}
				}
			}
